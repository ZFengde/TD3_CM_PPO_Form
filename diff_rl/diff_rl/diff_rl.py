...
class TD3(OffPolicyAlgorithm):
    ...
    def _setup_model(self):
        ...
        self.target_huber_distance = float(-np.sqrt(np.prod(self.env.action_space.shape)).astype(np.float32))
    ...
    def train(self, gradient_steps: int, batch_size: int = 100):
        ...
        for _ in range(gradient_steps):
            ...
            # huber_coef_loss = -(self.log_huber_coef * (huber_distance + 2 * self.target_huber_distance).detach()).mean()
            ...
            if self._n_updates % self.policy_delay == 0:
                ...
                # actor_loss = (bc_losses["bounded_cm_loss"] - self.critic.q1_forward(replay_data.observations, sampled_action) - huber_coef * huber_distance).mean()
                # actor_loss = (bc_losses["bounded_cm_loss"] - self.critic.q1_forward(replay_data.observations, sampled_action)).mean()
                actor_loss = bc_losses["bounded_cm_loss"]
